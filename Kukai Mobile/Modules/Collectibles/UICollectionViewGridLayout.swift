...
protocol UICollectionViewGridLayoutDelegate: AnyObject {
...
class UICollectionViewGridLayoutAttributes: UICollectionViewLayoutAttributes {
	var index: GridIndex?
}
...
class UICollectionViewGridLayout: UICollectionViewLayout {
	...
	private var attributes: [ElementKind: [IndexPath: UICollectionViewGridLayoutAttributes]] = [:]
	...
	private func prepareCells(at section: Int) {
		...
		while (itemsCount - itemIndex) > 0 {
			...
			(0..<numberOfColumns).forEach { column in
				...
				let attributes = UICollectionViewGridLayoutAttributes(forCellWith: indexPath)
	...
	private func prepareSuplementaryView(kind: ElementKind, at section: Int) {
		...
		let attributes = UICollectionViewGridLayoutAttributes(forSupplementaryViewOfKind: kind.rawValue, with: indexPath)
