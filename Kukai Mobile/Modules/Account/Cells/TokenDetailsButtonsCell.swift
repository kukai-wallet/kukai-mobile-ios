...
class TokenDetailsButtonsCell: UITableViewCell {
	...
	func setup(buttonData: TokenDetailsButtonData, moreMenu: UIMenu?, delegate: TokenDetailsButtonsCellDelegate?) {
		...
		favouriteButton.setImage( buttonData.isFavourited ? UIImage(named: "star-fill") : UIImage(named: "star-no-fill") , for: .normal)
	...
	@IBAction func favouriteButtonTapped(_ sender: Any) {
		...
		if let result = delegate?.favouriteTapped(){
			favouriteButton.setImage( result ? UIImage(named: "star-fill") : UIImage(named: "star-no-fill") , for: .normal)
		}
	}
