	...
	func updateAmountDisplay(withValue value: TokenAmount) {
		...
		if amountText.count > Int(UIScreen.main.bounds.width / 4) {
			...
			if (amountText.components(separatedBy: ".").first?.count ?? amountText.count) > 5 {
				largeDisplayAmount.text = DependencyManager.shared.coinGeckoService.formatLargeTokenDisplay(value.toNormalisedDecimal() ?? 0, decimalPlaces: token.decimalPlaces)
			} else {
